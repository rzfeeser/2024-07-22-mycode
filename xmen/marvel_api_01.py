...
def marvelcharcall(rand, keyhash, pubkey, lookmeup, args):
    uri_to_call = f"{API}?name={lookmeup}&ts={rand}&apikey={pubkey}&hash={keyhash}"
    
    if args.comic:
        uri_to_call = f"{uri_to_call}&comics={args.comic}"


    print(uri_to_call)
    r = requests.get(uri_to_call)  # send an HTTP GET to this location
...
def main():
    ...
    result = marvelcharcall(rand, keyhash, pubkey, "Wolverine", args)  # search for Wolverine
